...
public class Purchaser : IStoreListener
{
	...
	public void Restore()
	{
		_mStoreExtensionProvider.GetExtension<IAppleExtensions>().RestoreTransactions(success => { RestorePurchased?.Invoke(success); });
	}


	public Product GetProduct(string id) { return _mStoreController?.products.WithID(id); }


	public void OnInitializeFailed(InitializationFailureReason error)                { Debug.Log("OnInilized Failed"); }
